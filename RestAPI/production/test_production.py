...
try:
    ...
    print(f"Warning: Could not create test production orders: {e}")

def pytest_generate_tests(metafunc):
    if "input" in metafunc.fixturenames:
        metafunc.parametrize('url, test_description, input, expected_output', test_cases)

def test_production_endpoints(url, test_description, input, expected_output):
    """Test production endpoints with support for different HTTP methods"""
