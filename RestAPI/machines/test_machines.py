...
from .test_data_setup import create_test_machines
...
try:
    ...
    print(f"Warning: Could not create test machines: {e}")

def pytest_generate_tests(metafunc):
    if "input" in metafunc.fixturenames:
        metafunc.parametrize('url, test_description, input, expected_output', test_cases)

def test_machine_endpoints(url, test_description, input, expected_output):
    """Test machine endpoints with support for different HTTP methods"""
